...
- **Tests front automatises** : socle Vitest en place sur la logique pure
  alertes, cockpit demo, workflow case et demo investigation
  (`apps/web/lib/alerts-feed.ts`, `demo-cockpit.ts`, `case-workflow.ts`,
  `demo-investigation.ts`). Baseline Playwright etendue aux parcours cockpit,
  rings, score, alerts, cases, audit et upload. Les prochaines extensions
  utiles concernent surtout les composants critiques cote DOM et les parcours
  relies a un backend reel (OIDC, SSE durci, upload live FastAPI).
